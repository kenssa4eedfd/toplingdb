...
ifneq ($(origin CPU),undefined)
ifneq ($(origin CPU),environment)
  # CPU will pass through to sub make for sideplugins & topling core lib
  # so if it is   defined, it must be defined by environment
  #    if it is undefined, it will be auto detected both in this Makefile and sub makes
  $(error CPU=${CPU} is defined by $(origin CPU), it must keep undefined or define by environment)
endif
endif
...
ifeq ($(filter -DROCKSDB_JEMALLOC,$(PLATFORM_CXXFLAGS)),)
    PLATFORM_CXXFLAGS += -DTOPLING_DISABLE_JEMALLOC
    export TOPLING_DISABLE_JEMALLOC := 1
endif
...
${TOPLING_CORE_DIR}/${TOPLING_LIB_OBJ_LIST_FILE}: $(addprefix ${TOPLING_CORE_DIR}/, ${TOPLING_LIB_SRC_LIST_VAR})
...
rust-support: $(filter-out util/build_version.cc, ${LIB_SOURCES}) $(OBJ_DIR)/util/build_version_rough_build_time.cc
	...
	echo "" -DUSE_ZLIB=1 >> rust-rocksdb-cxxflags.txt
